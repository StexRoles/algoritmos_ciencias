class BPlusTree:
    ...
    def _dividir_hijo(self, padre, indice):
        ...
        nuevo_nodo[0] = hijo[0][medio:]
        hijo[0] = hijo[0][:medio]
        
        # Dividir hijos si no es hoja
        if hijo[1]:
            nuevo_nodo[1] = hijo[1][medio:]
            hijo[1] = hijo[1][:medio]
    ...
    def _eliminar(self, nodo, clave):
        ...
        if not nodo[1] and idx < len(nodo[0]) and nodo[0][idx] == clave:
            self._eliminar_de_hoja(nodo, idx)
        elif nodo[1]:  # Nodo interno
...
if __name__ == "__main__":
    ...
    print("\nÁrbol B+ después de eliminaciones:")
    arbol.mostrar_arbol()